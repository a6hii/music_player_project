...
class PlayerPlayListAddRemoveButton extends StatefulWidget {
  const PlayerPlayListAddRemoveButton({
    Key? key,
    required this.index,
  }) : super(key: key);
  final dynamic index;
  ...
  @override
  State<PlayerPlayListAddRemoveButton> createState() =>
      _PlayerPlayListAddRemoveButtonState();
}

class _PlayerPlayListAddRemoveButtonState
    extends State<PlayerPlayListAddRemoveButton> {
  @override
  Widget build(BuildContext context) {
    List<dynamic> selectedsongs = PlayListFunctions.ids[widget.index];
    final checkindex = selectedsongs.indexWhere(
      (element) => element == MainPage.selectedid,
    );
    final check = selectedsongs.contains(
      MainPage.selectedid,
    );
    if (check == true) {
      return IconButton(
          onPressed: () async {
            ...
            await PlayListFunctions.upgradeplaylist(
              widget.index,
              model,
            );
            setState(() {});
