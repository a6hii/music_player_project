...
class _AddSongIconState extends State<AddSongIcon> {
  ...
  Widget build(BuildContext context) {
    final check = AddSongFromPlayList.selectedsongs
        .contains(Musics.songslist[widget.index].id);
    if (check == true) {
      return IconButton(
          onPressed: () async {
            final checkindex = AddSongFromPlayList.selectedsongs.indexWhere(
                (element) => element == Musics.songslist[widget.index].id);
            await AddSongFromPlayList.selectedsongs.removeAt(checkindex);
            setState(() {});
          },
          icon: Icon(Icons.remove_circle,
              color: Themecolors.fontColor ?? Colors.white, size: 25));
    }
    return IconButton(
        onPressed: () async {
          AddSongFromPlayList.selectedsongs
